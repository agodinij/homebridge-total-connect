...
function errorResultCode(data, callback)
{
...
function totalConnect(log, config) {
...
totalConnect.prototype.login = function(callback, method, target)
{
    var that = this;
    ...
    request.post({
        ...
        if (!err && response.statusCode == 200) {
            ...
            var errorCode = errorResultCode(result, callback);
            if (errorCode == 1) {
                ...
                if (sessionID != null) {
                    ...
                    that.log('Got session: ' + that.sessionToken);
...
totalConnect.prototype.getSessionDetails = function(callback, method, that, target) {
    ...
    request.post({
        ...
        if (!err && response.statusCode == 200) {
            ...
            if(TotalConnectError == 1) {
                ...
                that.login(callback, method, target);
            }
...
totalConnect.prototype.getArmingState = function(callback) {
    this.login(callback, this.getArmingStateAuthenticated, null);
}

totalConnect.prototype.getArmingStateAuthenticated = function(callback, that, target) {
    //Call
    ...
    request.post({
        ...
        if (!err && response.statusCode == 200) {
            ...
            if(TotalConnectError == 1) {
                that.log('re-auth needed from response -102 in auth method...');
                that.login(callback, that.GetCurrentStateAuthenticated, null);
            }
            else if(TotalConnectError == 2){
                return;
            }
...
totalConnect.prototype.SetTargetState = function(callback, state) {
    var method = null;
    var target = null;
    switch (state) {
        case "away":
    ...
    this.login(callback, method, target);
}

totalConnect.prototype.ArmAuthenticated = function(callback, that, target) {
    ...
    request.post({
        ...
        if (!err && response.statusCode == 200) {
            ...
            var TotalConnectError = TotalConnectResultErrorCode(result, callback);
            if(TotalConnectError == 1) {
                that.log('Re-authentication needed from response "-102" in authenticaion method');
                that.login(callback, that.ArmAuthenticated, that.target);
            }
            else if(TotalConnectError == 2){
                return;
            }
...
totalConnect.prototype.DisarmAuthenticated = function(callback, that, target) {
